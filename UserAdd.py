...
def submit():
    ...
    if (password != "") and (Rpassword != "") and (street != "") and (house_no != "") and (city != "") and (
            f_name != "") and (l_name != ""):
        if password == Rpassword:
            ...
            executioner.execute("insert into cinema_management.customer_data values(%s,%s,%s,%s,%s,%s,%s)",
                                (customer_id, password, f_name, l_name, street, house_no, city))
            executioner.execute("commit")
            tkinter.messagebox.showinfo(title="Success", message=f"Data stored Successfully with {customer_id}")
            wind_sign_up.destroy()
            os.system("show_users.py")
        else:
            tkinter.messagebox.showerror(title="MIS-MATCH",
                                         message="ENTER SAME PASSWORDS IN PASSWORD RELATED FIELDS!!!")
    else:
...
# SIGN-UP header
header1 = Label(wind_sign_up, text="ADD-USER", font="Times 30 bold", fg="White", bg="black")
header1.pack()
...
bk_btn = Button(wind_sign_up, text="Back", fg="red", bg="Yellow", font="Times 20 bold",command=bk)
bk_btn.place(x=150,y=420)
